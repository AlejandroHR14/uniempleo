...
export const BannerFormFilter = (
  ...
  const handleCityClick = (city: any) => {
    if (!city) {
      ...
      elements.forEach((element) => {
        const cityId = element.getAttribute("id");

        const cityData = cities.find((c) => String(c.id) === cityId);

        if (cityData) {
          if (cityData.clickable) {
            element.classList.remove("selected-city");
            element.classList.add("default-city");
          } else {
            element.classList.remove("selected-city", "default-city");
            element.classList.add("non-clickable-city");
          }
        }
      });

      setfilterBody({ ...filterBody, city: null });
    } else {
      setfilterBody({ ...filterBody, city: city.name });
      ...
      elements.forEach((element) => {
        ...
        if (cityData) {
          if (cityData.id === city.id) {
            element.classList.remove("default-city");
            element.classList.add("selected-city");
          } else if (cityData.clickable) {
            element.classList.remove("selected-city");
            element.classList.add("default-city");
          } else {
            element.classList.remove("selected-city", "default-city");
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full">
      <div>
        <h2 id="city" className="font-semibold text-center">
          {filterBody.city ? cities.find((city: City) => city.id == filterBody.selectedCity)?.name : "Seleccione una ciudad"}
        </h2>
        <div className="flex items-center justify-center">
          ...
          >
            {cities.map((city) => {
              const fillColor = city.clickable
                ? filterBody.city === city.id
                  ? "#ecc27d" // Seleccionado y clickable
                  : "#1e3a58" // No seleccionado pero clickable
                : "#4b5563"; // No clickable (gris oscuro)

              // Determinar si la clase hover y el cursor se aplican
              const isClickableClass = city.clickable
                ? "cursor-pointer hover:fill-[#ecc27d] transition-all duration-300 ease-in-out"
                : "cursor-not-allowed";

              return (
                <path
                  key={isMobile ? "Mobile" + city.name : city.name}
                  id={isMobile ? "Mobile" + city.id : city.id}
                  d={city.d}
                  onClick={() => city.clickable && handleCityClick(city)} // Solo permite clic si es clickeable
                  className={isClickableClass}
                  fill={fillColor} // Aplica el color de relleno dinámicamente
                />
              );
            })}
      ...
      {/* <InputText
        ...
        placeholder="Empresa o Cargo a Buscar"
        className="w-full"
      /> */}
      <button
        type="submit"
